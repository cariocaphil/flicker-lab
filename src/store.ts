...
const resizeFrameCells = (
  cells: Cell[][],
  newResolution: Resolution
): Cell[][] => {
  const oldRes = Math.sqrt(cells.length);
  const newCells = Array(newResolution)
    .fill(null)
    .map(() => Array(newResolution).fill(0));
...
export const useFlickerStore = create<FlickerStore>((set, get) => {
  ...
  return {
    ...
    setFrameResolution: (frameIndex, resolution) =>
      set((state) => {
        const newSequence = [...state.sequence];
        const frame = { ...newSequence[frameIndex] };
      ...
      set((state) => {
        ...
        for (let i = start; i <= end; i++) {
          if (i + (end - start + 1) < newSequence.length) {
            newSequence[i + (end - start + 1)] = JSON.parse(
              JSON.stringify(newSequence[i])
            );
          }
        }
        const newState = { sequence: newSequence };
